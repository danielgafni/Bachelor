...
\end{subfigure}
\caption{Visualization of $XY$ connection weights of a 900 $Y$ neurons (100 neurons for each of the 9 patches) network --- a 360 $ \times $ 360 matrix. The weights of еру one neuron correspond to a 12 $\times$ 12 square. The weights of the $Y$ neurons are grouped by the patch --- the lower left 120 $  \times $ 120 square  (10 $ \times $ 10 neurons) corresponds to the lower left patch, the center square corresponds to the center patch, and so on. Weights are initialized randomly (\subrefbf{fig:training:before_training}), and after 1000 training iterations (\subrefbf{fig:training:after_training}) the neurons learnin different features, resembling digits elements}
\label{fig:training}
\end{figure}
