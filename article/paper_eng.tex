...
\end{subfigure}
\caption{The effect of competition on $ XY $ connections training. (\subref{fig:competition_effect_on_training:low_comp}) shows poorly specialized neuros with similar features - the result of training with competiiton weights set to $-10$, while (\subref{fig:competition_effect_on_training:high_comp}) features highly specialized neurons with diverse features, trained with inhibitory weights set to $-100$}
...
\hline
\ref{fig:competition_distributions:weakest}, \ref{fig:worst_competition_distribution} & 14.7 & 14.2 & -0.5 & 1.5 \\
\hline
\ref{fig:competition_distributions:weak}, \ref{fig:medium_bad_competition_distribution} & 5.4 & 15.1 & -1.2 & 0.6 \\
\hline
\ref{fig:competition_distributions:medium}, \ref{fig:medium_good_competition_distribution} & 17.6 & 24.5 & -1.9 & 1.6 \\
\hline
\ref{fig:competition_distributions:strong}, \ref{fig:best_competition_XY} & 17.7 & 16.5 & -0.1 & 1.6\\
\hline
\end{tabular}
\end{center}
\end{table}
...
\end{subfigure}
\caption{Different inhibitory ($YY$) weights distributions: weakest competition (\subref{fig:competition_distributions:weakest}), weak competition (\subref{fig:competition_distributions:weak}), medium competition (\subref{fig:competition_distributions:medium}) and strong competition (\subref{fig:competition_distributions:strong})}
...
\end{subfigure}
\caption{Visualization of inhibitory weights on top of $ XY $ weights for the networks from Fig. (\ref{fig:competition_distributions}). Inhibitory weights are shown only for one center neuron in each receptive field to avoid cluttering the visualization. The saturated blue color corresponds to the inhibitory weights of large absolute value (the arithmetic mean between the weights $ W_{ij} $ and $ W_{ji} $ is used). Figures \subref{fig:worst_competition_distribution}-\subref{fig:best_competition_XY} correspond to the same networks as on Figures \ref{fig:competition_distributions:weakest}-\subref{fig:competition_distributions:strong} (from weakest to strongest competition). It can be seen that similar features have higher inhibitory connections weights than different}
\end{figure}
