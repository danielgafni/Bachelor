...
Experiments were carried out to measure the accuracy of networks with different architectures. For convolutional and fully connected networks,  competition connections similar to LCSNN were introduced. Note that using a linear classifier as an interpretation algorithm, it was possible to achieve 95\% accuracy for a locally connected network of 1000 channels with kernel size of 12.
...
\footnotetext[3]{Network with trainable competition connections}
...
\end{subfigure} 
\caption{Impact of limiting the values of competition weights on accuracy. The weights were limited to $ -350 $ on the illustration below and $ -100 $ on the illustration above. The accuracy of a network with similar parameters, but without competition limitation is 0.823, its weights $ XY $ are shown in Fig. \ref {fig:best_competition_XY}, and the distribution of its competition weights is shown in Fig. \ref{fig:best_competition}. It can be seen that in both cases more blurred forward propagation weights are trained.
}
\label{fig:compe_clamp}
\end{figure}

Competition training made it possible to achieve an accuracy slightly (by 2 \%) higher than the accuracy of a network of the same configuration, but with a fixed competition with competition weights equal to $ -50 $ (Table \ref{results}, №4 and №5).
...
It was also shown that training competition weights can slightly improve the quality of the model, and in the resulting distribution of competition weights, both with high values (strong competition) and low (cooperation) values of weights are important.
